...
def del_hour_cmd(
    ...
    with uow:
        ...
        # Check if the hour doesn't exist
        hours: List[int] = [gp_hour.hour for gp_hour in uow.repo.get_hours(chat_id=chat_id)]
        if hour not in hours:
            text = f"La hora {hour} no existe en el itinerario"
            update.effective_message.reply_text(text=text)
            return
            
        uow.repo.del_hour(chat_id=chat_id, hour=hour)
        text = f"Se borro la hora {hour} del itinerario"
        update.effective_message.reply_text(text=text)
